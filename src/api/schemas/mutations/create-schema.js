...
export default async (root, { name, endpoint }, { db, user }) => {
  if (!user) throw new Error('Access denied');

  const schemasCount = await db.gqlSchemasConnection({ where: { owner: { id: user.id } } })
    .aggregate()
    .count();
  ...
  const schema = await db.createGqlSchema({
